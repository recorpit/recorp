// src/app/(dashboard)/impostazioni/anagrafiche/committenti/page.tsx
'use client'
...
const OPTIONAL_COLUMNS = [
  'codiceFiscale', 'email', 'pec', 'telefono', 'codiceSDI',
  'indirizzoFatturazione', 'capFatturazione', 'cittaFatturazione', 'provinciaFatturazione',
  'quotaAgenzia', 'giorniPagamento', 'iban', 'aRischio', 'note'
]

export default function ImportCommittentiPage() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/api/import/committenti/preview', {
        method: 'POST',
        body: formData,
      })
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/import/committenti', {
        method: 'POST',
        body: formData,
      })
  ...
  const handleExportTemplate = async () => {
    const res = await fetch('/api/import/committenti/template')
    const blob = await res.blob()
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'template_committenti.xlsx'
    a.click()
    window.URL.revokeObjectURL(url)
  }

  const handleExportAll = async () => {
    const res = await fetch('/api/export/committenti')
    const blob = await res.blob()
    const url = window.URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `committenti_export_${new Date().toISOString().split('T')[0]}.xlsx`
    a.click()
    window.URL.revokeObjectURL(url)
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Import Committenti</h1>
            <p className="text-gray-500">Carica file Excel o CSV con i dati dei committenti</p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-gray-700">
                ✓ {result.imported} nuovi committenti importati
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span>Solo nuovi (salta duplicati P.IVA)</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={mode === 'update'} onChange={() => setMode('update')} />
                <span>Aggiorna esistenti (match per P.IVA)</span>
              </label>
            </div>
          </div>
